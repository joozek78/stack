...
data Archive = Archive
    { archiveMappings :: !(Map Text Mapping)
...
data Mapping = Mapping
    { mappingSigners :: !(Map Signer (Set PackageName))
    } deriving (Show)
...
instance FromJSON (Map Signer (Set PackageName)) where
    parseJSON j = do
        ...
        fmap
            ...
            (mapM
                 (\o ->
                       do fingerprint <- o .: "fingerprint"
                          email <-
                              fmap unAeson (o .: "email")
                          packages <-
                              fmap
                                  (S.map unAeson)
                                  (o .: "packages")
...
-- | A package signer.
data Signer = Signer
    { signerFingerprint :: !FingerprintSample
...
instance ToJSON Signer where
    toJSON (Signer fingerprint email) = object
...
newtype FingerprintSample = FingerprintSample
